...
const SKILL_LEARNING_DATA: Record<
  string,
  {
    description: string;
    roadmap: {
...
> = {
  Python: {
    description:
      "A versatile programming language essential for data science, web development, and automation.",
    roadmap: {
      beginner: [
    ...
    resources: [
      { name: "Python Official Docs", type: "documentation", url: "https://docs.python.org/3/" },
      {
        ...
        type: "video",
        url: "https://www.freecodecamp.org/news/learn-python-basics-in-4-hours/",
      },
  ...
  JavaScript: {
    ...
    resources: [
      ...
      {
        name: "Traversy Media JS Tutorials",
        type: "video",
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-muted-foreground">
              We found {skills.length} required skill{skills.length > 1 ? "s" : ""}. Click to
          ...
          <div className="flex flex-wrap gap-3">
            {skills.map((skill, index) => (
              <motion.button
                ...
                className={`rounded-full px-5 py-2 text-sm font-semibold transition-all duration-300 hover:scale-105 active:scale-95 ${
                  expandedSkill === skill
          ...
          <AnimatePresence>
            {expandedSkill && (
              ...
              >
                <div className="border-border/50 space-y-6 border-t pt-6">
                  {(() => {
                    const skillData = getSkillData(expandedSkill);
                    return (
                      <>
                        <div>
                          <h4 className="font-heading mb-3 text-xl font-semibold">
                            {expandedSkill}
                          </h4>
                          <p className="text-muted-foreground mb-4">{skillData.description}</p>
                        </div>
                          ...
                          <div className="grid gap-3">
                            {skillData.resources.map((resource, i) => (
                              <a
                                ...
                                className="bg-card border-border/50 hover:border-primary/30 hover:shadow-soft group flex items-center justify-between rounded-2xl border p-4 transition-all duration-300"
                              >
